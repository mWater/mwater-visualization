          ...
          cleanSegment = function cleanSegment(segment) {
            if (segment.valueAxis) {
              segment.valueAxis = axisBuilder.cleanAxis({
                axis: original(segment.valueAxis),
                table: design.table,
                aggrNeed: "none",
            ...
            if (segment.filter) {
              segment.filter = exprCleaner.cleanExpr(original(segment.filter), {
                table: design.table,
                types: ["boolean"]
              });
            }
            ...
            if (segment.orderExpr) {
              return segment.orderExpr = exprCleaner.cleanExpr(original(segment.orderExpr), {
                table: design.table,
          ...
          for (intersectionId in ref2) {
            ...
            if (intersection.valueAxis) {
              intersection.valueAxis = axisBuilder.cleanAxis({
                axis: original(intersection.valueAxis),
                table: design.table,
                aggrNeed: "required",
            ...
            if (intersection.backgroundColorAxis) {
              intersection.backgroundColorAxis = axisBuilder.cleanAxis({
                axis: original(intersection.backgroundColorAxis),
                table: design.table,
                aggrNeed: "required",
                types: ["enum", "text", "boolean", "date"]
              });
            ...
            if (intersection.filter) {
              intersection.filter = exprCleaner.cleanExpr(original(intersection.filter), {
                table: design.table,
                types: ["boolean"]
              });
            }
