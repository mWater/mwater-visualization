...
export default DatagridViewComponent = (function() {
  DatagridViewComponent = class DatagridViewComponent extends React.Component {
    ...
    handleSaveEdit() {
      ...
      const {
        expr
      } = this.props.design.columns[this.state.editingCell.columnIndex];
    ...
    renderCell(column, columnIndex, exprType, cellProps) {
      ...
      if ((this.state.editingCell?.rowIndex === cellProps.rowIndex) && (this.state.editingCell?.columnIndex === columnIndex)) {
        // Special if saving
        if (this.state.savingCell) {
          return R(Cell, cellProps,
            R('i', {className: "fa fa-spinner fa-spin"}));
    ...
    renderColumn(column, columnIndex) {
      ...
      return R(Column, {
        key: column.id,
        header: R(Cell, {style: { whiteSpace: "nowrap" }}, column.label || exprUtils.summarizeExpr(column.expr, this.props.design.locale)),
        width: column.width,
