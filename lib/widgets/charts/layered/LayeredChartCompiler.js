...
var AxisBuilder, ExprCompiler, ExprUtils, LayeredChartCompiler, _, calculateLinearRegression, cleanString, commaFormatter, compactFormatter, d3Format, defaultColors, injectTableAlias, labelValueFormatter, pieLabelValueFormatter;
...
pieLabelValueFormatter = function pieLabelValueFormatter(format) {
  var hidePercent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var percent;

  percent = function percent(ratio) {
    if (hidePercent) {
      return "";
    } else {
      return "(".concat(d3Format.format('.1%')(ratio), ")");
    }
  };

  return function (value, ratio, id) {
    if (format[id]) {
      return "".concat(format[id](value), " ").concat(percent(ratio));
    } else {
      return "".concat(d3Format.format(",")(value), " ").concat(percent(ratio));
    }
  };
};
...
defaultColors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf', '#aec7e8', '#ffbb78', '#98df8a', '#ff9896', '#c5b0d5', '#c49c94', '#f7b6d2', '#c7c7c7', '#dbdb8d', '#9edae5']; // Compiles various parts of a layered chart (line, bar, scatter, spline, area) to C3.js format
...
module.exports = LayeredChartCompiler = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(LayeredChartCompiler, [{
    ...
    value: function createChartOptions(options) {
      ...
      tickFormatter = _.keys(c3Data.format).length > 0 ? c3Data.format[_.keys(c3Data.format)[0]] : commaFormatter;
      ...
      chartDesign = {
        ...
        pie: {
          label: {
            ...
            format: options.design.labels ? pieLabelValueFormatter(c3Data.format, options.design.hidePercentage) : void 0
          },
    ...
    value: function compileDataNonCategorical(design, data, locale) {
      ...
      var classes, colors, columns, dataMap, format, groups, legendHide, names, types, xType, xs;
      columns = [];
      types = {};
      names = {};
      dataMap = {};
      ...
      _.each(design.layers, function (layer, layerIndex) {
        var categories, categoryOrder, colorValues, layerData, seriesX, seriesY, trendlineSeries, xValues, yValues; // Get data of layer
        ...
        if (layer.axes.color) {
          ...
          _.each(layerData, function (row, rowIndex) {
            return dataMap["".concat(seriesY, ":").concat(rowIndex)] = {
              layerIndex: layerIndex,
              row: row
            };
          ...
          if (layer.trendline === "linear") {
            ...
            format[trendlineSeries] = function (value) {
              if (value != null) {
                return _this2.axisBuilder.formatValue(layer.axes.y, value, locale, true);
              } else {
                return "";
    ...
    value: function compileDataCategorical(design, data, locale) {
      ...
      var categories, categoryMap, categoryOrder, categoryXs, classes, colorOverrides, colors, column, columns, dataMap, defaultStacked, format, groups, i, j, k, layer, layerIndex, legendHide, len, len1, len2, m, names, nullLabel, o, p, ref, ref1, ref2, stacked, types, xAxis, xType, xValues, xs, xtotals;
      columns = [];
      types = {};
      names = {};
      dataMap = {};
      ...
      nullLabel = _.first(_.compact(design.layers.map(function (l) {
        return l.axes.x.nullLabel;
      })));
      ...
      _.each(design.layers, function (layer, layerIndex) {
        var colorCategories, colorCategoryOrder, colorValues, column, layerData, series, trendXs, trendYs, trendlineSeries; // Get data of layer
        ...
        if (layer.axes.color) {
          ...
          if (colorValues.length > 0) {
            ...
            series = "".concat(layerIndex, ":dumm");
            column = _.map(categories, function (c) {
              return null;
            });
            columns.push([series].concat(column));
            types[series] = _this3.getLayerType(design, layerIndex);
            names[series] = _this3.axisBuilder.formatValue(layer.axes.color, null, locale, true);
            xs[series] = "x";
            return format[series] = function (value) {
              if (value != null) {
                return _this3.axisBuilder.formatValue(layer.axes.y, value, locale, true);
          ...
          if (layer.trendline === "linear") {
            ...
            trendYs = column; // Skip null last value
      ...
      if (design.proportional) {
        ...
        for (o = 0, len2 = columns.length; o < len2; o++) {
          column = columns[o]; // Skip x column

          if (column[0] === 'x') {
            continue;
          }

          for (i = p = 1, ref2 = column.length; 1 <= ref2 ? p < ref2 : p > ref2; i = 1 <= ref2 ? ++p : --p) {
            if (column[i] > 0) {
      ...
      return {
        ...
        color: function color(_color, d) {
          var key, len3, q, sortedYThresholds, yThreshold; // Handle overall series color which calls with a non-object for d
          ...
          for (q = 0, len3 = sortedYThresholds.length; q < len3; q++) {
            yThreshold = sortedYThresholds[q];
...
calculateLinearRegression = function calculateLinearRegression(ys, xs) {
  var den, intercept, n, nonNullxs, nonNullys, slope, sumX, sumXX, sumXY, sumY; // If xs are dates, convert to numbers

  if (_.isString(xs[0])) {
    xs = _.map(xs, function (x) {
      return Date.parse(x);
    });
  } // Remove null ys


  nonNullxs = _.filter(xs, function (x, index) {
    return ys[index] !== null;
  });
  nonNullys = _.filter(ys, function (y, index) {
    return ys[index] !== null;
  });
  n = nonNullys.length;
  sumXY = _.sum(_.map(nonNullxs, function (x, i) {
    return x * nonNullys[i];
  }));
  sumXX = _.sum(_.map(nonNullxs, function (x) {
    return x * x;
  }));
  sumX = _.sum(nonNullxs);
  sumY = _.sum(nonNullys); // Calculate denominator

  den = n * sumXX - sumX * sumX; // Calculate slope

  slope = (n * sumXY - sumX * sumY) / den; // Calculate intercept
