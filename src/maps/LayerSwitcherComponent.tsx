...
export function LayerSwitcherComponent(props: {
  ...
  function renderLayerView(lv: MapLayerView, index: number) {
    function handleClick() {
      props.onDesignChange(produce(props.design, draft => {
    ...
    return <div key={index} style={{ fontSize: 12, whiteSpace: "nowrap", cursor: "pointer" }} onClick={handleClick}>
      <i className={ lv.visible ? "fa fa-fw fa-check-square text-primary" : "fa fa-fw fa-square text-muted" }/>&nbsp;{lv.name}
    </div>
  }

  return <div style={{ position: "absolute", top: 20, right: 20, zIndex: 1000, userSelect: "none" }} ref={useClickOutside(() => { setDropdownOpen(false) })}>
