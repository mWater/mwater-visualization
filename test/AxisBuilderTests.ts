...
describe("AxisBuilder", function() {
  ...
  describe("cleanAxis", function() {
    ...
    it("remove if not possible to get type", function() {
      ...
      assert(this.ab.cleanAxis({axis, table: "t1", types: ['enum'], aggrNeed: "none"}), "Number can be binned to enum"); // Can get enum via binning
    
      return assert(!this.ab.cleanAxis({axis: { expr: this.exprText }, table: "t1", types: ['number'], aggrNeed: "none"}), "No aggr allowed");
    });
  ...
  describe("formatValue", function() {
    ...
    it("formats enum overridden", function() {
      return assert.equal(this.ab.formatValue(_.extend({}, this.axisEnum, { categoryLabels: { '"a"': "A2"}}), "a"), "A2");
    });
    ...
    it("formats enumset overridden", function() {
      return assert.equal(this.ab.formatValue(_.extend({}, this.axisEnumset, { categoryLabels: { '"a"': "A2"}}), ["a", "b"]), "A2, B");
    });
    ...
    it("wraps text[]", function() {
      compare(this.ab.formatValue(this.axisTextarr, ["a", "b"]), R('div', null, R('div', {key: 0}, "a"), R('div', {key: 1}, "b")));
      return compare(this.ab.formatValue(this.axisTextarr, JSON.stringify(["a", "b"])), R('div', null, R('div', {key: 0}, "a"), R('div', {key: 1}, "b")));
    });
