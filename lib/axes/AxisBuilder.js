...
class AxisBuilder {
    ...
    constructor(options) {
        ...
        this.exprUtils = new mwater_expressions_3.ExprUtils(this.schema);
        this.exprCleaner = new mwater_expressions_4.ExprCleaner(this.schema);
        this.exprValidator = new mwater_expressions_2.ExprValidator(this.schema);
    }
    ...
    */
    cleanAxis(options) {
        ...
        axis = immer_1.default(axis, (draft) => {
            draft.expr = expr;
            if (axis.aggr) {
                delete draft.aggr;
            }
            ...
            if (draft.xform && draft.xform.type === "ranges") {
                ...
                if (!draft.xform.ranges) {
                    draft.xform.ranges = [{ id: uuid_1.default(), minOpen: false, maxOpen: true }];
                }
            }
        });
        return axis;
    ...
    */
    ...
    getCategories(axis, values, options = {}) {
        ...
        if (axis.xform && axis.xform.type === "year") {
            ...
            if (options.onlyValuesPresent) {
                ...
                categories = lodash_1.default.sortBy(values, item => item).map(value => ({ value, label: value.substring(0, 4) }));
                if (hasNone) {
                    categories.push(noneCategory);
                }
                return categories;
        ...
        if (axis.xform && axis.xform.type === "yearmonth") {
            ...
            if (options.onlyValuesPresent) {
                // Sort and take only present
                categories = lodash_1.default.sortBy(values, item => item).map(value => ({ value, label: moment_1.default(value, "YYYY-MM-DD").format("MMM YYYY") }));
                if (hasNone) {
                    categories.push(noneCategory);
                }
                return categories;
            ...
            current = moment_1.default(min, "YYYY-MM-DD");
            end = moment_1.default(max, "YYYY-MM-DD");
            categories = [];
        ...
        if (axis.xform && axis.xform.type === "yearweek") {
            ...
            if (options.onlyValuesPresent) {
                // Sort and take only present
                categories = lodash_1.default.sortBy(values, item => item).map(value => ({ value, label: value }));
                if (hasNone) {
                    categories.push(noneCategory);
                }
                return categories;
            ...
            max = values.sort().slice(-1)[0];
            // Use moment to get range
            current = moment_1.default(min, "GGGG-WW");
            end = moment_1.default(max, "GGGG-WW");
            categories = [];
            while (!current.isAfter(end)) {
        ...
        if (axis.xform && axis.xform.type === "yearquarter") {
            ...
            if (options.onlyValuesPresent) {
                // Sort and take only present
                categories = lodash_1.default.sortBy(values, item => item).map(value => {
            ...
            max = values.sort().slice(-1)[0];
            // Use moment to get range
            current = moment_1.default(min, "YYYY-Q");
            end = moment_1.default(max, "YYYY-Q");
            categories = [];
            while (!current.isAfter(end)) {
        ...
        switch (this.getAxisType(axis)) {
                ...
                if (options.onlyValuesPresent) {
                    ...
                    for (const ev of this.exprUtils.getExprEnumValues(axis.expr)) {
                        if (flatValues.includes(ev.id)) {
                            categories.push({
                                value: ev.id,
                                label: mwater_expressions_3.ExprUtils.localizeString(ev.name, options.locale)
                            });
                        }
                    }
                    if (hasNone) {
            ...
            case "date":
                ...
                if (options.onlyValuesPresent) {
                    // Sort and take only present
                    categories = lodash_1.default.sortBy(values, item => item).map(value => ({ value, label: moment_1.default(value, "YYYY-MM-DD").format("ll") }));
                    if (hasNone) {
                        categories.push(noneCategory);
                    }
                    return categories;
    ...
    doesAxisSupportCumulative(axis) {
        ...
        return axisType == "date" || axisType == "number" || ((_a = axis.xform) === null || _a === void 0 ? void 0 : _a.type) === "yearquarter";
    }
    ...
    formatValue(axis, value, locale, legacyPercentFormat) {
        if (value == null) {
            return (axis === null || axis === void 0 ? void 0 : axis.nullLabel) || "None";
        }
        ...
        switch (type) {
            ...
            case "number":
                ...
                return valueFormatter_1.formatValue(type, num, axis.format, locale, legacyPercentFormat);
            case "geometry":
                return valueFormatter_1.formatValue(type, value, axis.format, locale, legacyPercentFormat);
            case "text[]":
                // Parse if string
                if (lodash_1.default.isString(value)) {
                    value = JSON.parse(value);
