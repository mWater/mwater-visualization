...
calculateLinearRegression = function calculateLinearRegression(ys, xs) {
  var den, intercept, n, slope, sumX, sumXX, sumXY, sumY; // If xs are dates, convert to numbers
  ...
  xs = _.filter(xs, function (x, index) {
    return ys[index] !== null;
  });
  ys = _.filter(ys, function (y, index) {
    return ys[index] !== null;
  });
  n = ys.length;
  sumXY = _.sum(_.map(xs, function (x, i) {
    return x * ys[i];
  }));
  sumXX = _.sum(_.map(xs, function (x) {
    return x * x;
  }));
  sumX = _.sum(xs);
  sumY = _.sum(ys); // Calculate denominator
