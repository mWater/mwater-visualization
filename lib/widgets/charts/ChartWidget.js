var ActionCancelModalComponent, ChartViewComponent, ChartWidget, ChartWidgetComponent, CsvBuilder, DropdownWidgetComponent, H, ModalWindowComponent, React, Widget,
...
ChartWidgetComponent = (function(superClass) {
  ...
  ChartWidgetComponent.prototype.handleSaveCsvFile = function() {
    return this.props.widgetDataSource.getData(this.props.filters, (function(_this) {
      return function(err, data) {
        var blob, csv, filesaver, table;
        if (err) {
          return alert("Failed to get data");
        }
        table = _this.props.chart.createDataTable(_this.props.design, _this.props.schema, data, _this.context.locale);
        if (!table) {
          return;
        }
  ...
  ChartWidgetComponent.prototype.renderChart = function(design, width, height, standardWidth) {
  ...
  ChartWidgetComponent.prototype.renderEditor = function() {
    ...
    chart = this.renderChart(this.state.editDesign, width, this.props.height * (width / this.props.width), width);
    content = H.div({
      style: {
        height: "100%",
        width: "100%"
  ...
  ChartWidgetComponent.prototype.render = function() {
    var dropdownItems, emptyDesign, validDesign;
    validDesign = !this.props.chart.validateDesign(this.props.chart.cleanDesign(this.props.design, this.props.schema), this.props.schema);
    emptyDesign = this.props.chart.isEmpty(this.props.design);
    dropdownItems = this.props.chart.createDropdownItems(this.props.design, this.props.schema, this.props.widgetDataSource, this.props.filters);
    if (validDesign) {
      dropdownItems.push({
        label: "Export Data",
        icon: "save-file",
    ...
    }, this.renderChart(this.props.design, this.props.width, this.props.height, this.props.standardWidth)), (emptyDesign || !validDesign) && (this.props.onDesignChange != null) ? this.renderEditLink() : void 0);
  };
