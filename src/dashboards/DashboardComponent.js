...
export default DashboardComponent = (function() {
  DashboardComponent = class DashboardComponent extends React.Component {
    ...
    renderEditingSwitch() {
      return R('a', {key: "edit", className: `btn btn-primary btn-sm ${this.state.editing ? "active" : ""}`, onClick: this.handleToggleEditing},
    ...
    renderStyle() {
      return R('button', {type: "button", key: "style", className: "btn btn-link btn-sm", onClick: this.handleOpenLayoutOptions},
    ...
    renderActionLinks() {
      return R('div', null,
        this.state.editing && ((this.props.design.layout || "grid") === "grid") ?
          R('a', {key: "upgrade", className: "btn btn-info btn-sm", onClick: this.handleUpgrade},
            "Upgrade Dashboard...") : undefined,
        this.state.editing ?
          [
        ...
        R('a', {key: "print", className: "btn btn-link btn-sm", onClick: this.handlePrint},
          R('span', {className: "glyphicon glyphicon-print"}),
          R('span', {className: "hide-600px"}, " Print")),
        R('a', {key: "refresh", className: "btn btn-link btn-sm", onClick: this.handleRefreshData},
        ...
        this.state.hideQuickfilters && this.props.design.quickfilters && (this.props.design.quickfilters.length > 0) ?
          R('a', {key: "showQuickfilters", className: "btn btn-link btn-sm", onClick: this.handleShowQuickfilters},
            R('span', {className: "fa fa-filter"}),
            R('span', {className: "hide-600px"}, " Show Filters")) : undefined,
        ...
        //   " Export"
        this.state.editing ?
          R('a', {key: "settings", className: "btn btn-link btn-sm", onClick: this.handleSettings},
            R('span', {className: "glyphicon glyphicon-cog"}),
            R('span', {className: "hide-600px"}, " Settings")) : undefined,
        this.state.editing ?
          this.renderStyle() : undefined,
        this.props.extraTitleButtonsElem,
        (this.props.onDesignChange != null) ?
          this.renderEditingSwitch() : undefined
      );
    }

    renderTitleBar() {
      return R('div', {style: { height: 40, padding: 4 }},
    ...
    render() {
      ...
      return R('div', {style: { display: "grid", gridTemplateRows: (this.props.hideTitleBar ? "auto 1fr" : "auto auto 1fr"), height: "100%" }},
        !this.props.hideTitleBar ?
          this.renderTitleBar() : undefined,
        R('div', null,
          !this.state.hideQuickfilters ?
            this.renderQuickfilter() : undefined
        ),
        dashboardView,
        (this.props.onDesignChange != null) ?
          R(SettingsModalComponent, { onDesignChange: this.handleDesignChange, schema: this.props.schema, dataSource: this.props.dataSource, ref: c => { return this.settings = c; } }) : undefined,
