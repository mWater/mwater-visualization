...
export default DatagridQueryBuilder = class DatagridQueryBuilder { 
  ...
  createComplexMainQuery(design, options) {
    ...
    let selects = [
      // Primary key
      { type: "select", expr: { type: "field", tableAlias: "main", column: this.schema.getTable(design.table).primaryKey }, alias: "id" },
    ...
    // Add columns
    selects = selects.concat(_.map(design.columns, (column, columnIndex) => this.createColumnSelect(column, columnIndex)));
  ...
  createComplexSubtableQuery(design, options, subtable, subtableIndex) {
    ...
    // Add columns
    selects = selects.concat(_.map(design.columns, (column, columnIndex) => this.createColumnSelect(column, columnIndex, subtable, options.fillSubtableRows)));
  ...
  getMainOrderByExprs(design, isAggr = false) {
    ...
    if (!isAggr) {
      ...
      const {
        ordering
      } = this.schema.getTable(design.table);
      if (ordering) {
  ...
  getSubtableOrderByExprs(design, subtable) {
    ...
    const {
      ordering
    } = this.schema.getTable(subtableTable);
    if (ordering) {
  ...
  createColumnSelect(column, columnIndex, subtable, fillSubtableRows) {
    ...
    // Compile expression
    const exprCompiler = new ExprCompiler(this.schema);
    let compiledExpr = exprCompiler.compileExpr({expr: exprCleaner.cleanExpr(column.expr, {aggrStatuses: ["individual", "literal", "aggregate"]}), tableAlias: column.subtable ? "st" : "main"});
  ...
  createSimpleSelects(design, isAggr) {
    ...
    if (!isAggr) {
      selects.push({ type: "select", expr: { type: "field", tableAlias: "main", column: this.schema.getTable(design.table).primaryKey }, alias: "id" });
    }
  ...
  createNullExpr(exprType) {
    switch (exprType) {
      ...
      case "text": case "enum": case "geometry": case "id": case "date": case "datetime":
        return { type: "op", op: "::text", exprs: [null] }; 
      case "boolean":
        return { type: "op", op: "::boolean", exprs: [null] }; 
      case "number":
        return { type: "op", op: "::decimal", exprs: [null] }; 
      case "enumset": case "text[]": case "image": case "imagelist":
