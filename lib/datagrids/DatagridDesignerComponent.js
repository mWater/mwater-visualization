...
class DatagridOptionsComponent extends react_1.default.Component {
    render() {
        return R(ui.FormGroup, { label: "Display Options" }, R(ui.Checkbox, {
            value: this.props.design.showRowNumbers,
            onChange: (showRowNumbers) => this.props.onDesignChange((0, update_object_1.default)(this.props.design, { showRowNumbers: { $set: showRowNumbers } }))
        }, "Show row numbers"));
    }
}
// Columns list
class ColumnsDesignerComponent extends react_1.default.Component {
    ...
    render() {
        ...
        }, R("span", { className: "fas fa-plus" }), " Add Default Columns"), R("button", {
            key: "removeAll",
            type: "button",
            className: "btn btn-link btn-sm",
            onClick: this.handleRemoveAllColumns
        }, R("span", { className: "fas fa-times" }), " Remove All Columns")), R(ReorderableListComponent_1.default, {
        ...
        }, R("span", { className: "fas fa-plus" }), " Add Column"), R("button", {
            key: "add-id",
            type: "button",
            className: "btn btn-link",
            onClick: this.handleAddIdColumn
        }, R("span", { className: "fas fa-plus" }), " Add Unique Id (advanced)")));
    }
}
// Column item
class ColumnDesignerComponent extends react_1.default.Component {
    constructor() {
        ...
        this.render = () => {
            ...
            const elem = R("div", { className: "row" }, R("div", { className: "col-1" }, this.props.connectDragSource(R("span", { className: "text-muted fa fa-bars" }))), R("div", { className: "col-6" }, // style: { border: "solid 1px #DDD", padding: 4 },
    ...
    renderSplit() {
        ...
        switch (exprType) {
            case "enumset":
                return R("a", { className: "btn btn-sm btn-link", onClick: this.handleSplitEnumset }, R("i", { className: "fa fa-chain-broken" }), " Split by options");
                break;
            case "geometry":
                return R("a", { className: "btn btn-sm btn-link", onClick: this.handleSplitGeometry }, R("i", { className: "fa fa-chain-broken" }), " Split by lat/lng");
                break;
        }
        return null;
    }
