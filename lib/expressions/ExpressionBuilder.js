...
module.exports = ExpressionBuilder = (function() {
  ...
  ExpressionBuilder.prototype.summarizeAggrExpr = function(expr, aggr) {
    var aggrName, summary;
    if (expr && !this.getExprType(expr) && aggr === "count") {
      return summary = "Number of " + (this.schema.getTable(expr.table).name);
    } else if (!aggr) {
      return summary = this.summarizeExpr(expr);
    } else {
