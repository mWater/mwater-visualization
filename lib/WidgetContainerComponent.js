...
Container = (function(superClass) {
  ...
  Container.prototype.renderLayouts = function() {
    var hoveredDragInfo, hoveredLayout, hoveredRect, id, layout, layouts, renderElems;
    renderElems = [];
    ...
    for (id in layouts) {
      layout = layouts[id];
      if (!hoveredLayout || id !== hoveredDragInfo.id) {
        renderElems.push(this.renderLayout(id, layout));
      } else {
        renderElems.push(this.renderPlaceholder(this.props.layoutEngine.getLayoutBounds(hoveredLayout)));
      }
    }
    return renderElems;
  };
