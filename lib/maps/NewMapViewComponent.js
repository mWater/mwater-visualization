...
function NewMapViewComponent(props) {
    const [mapDiv, setMapDiv] = react_1.useState(null);
    ...
    // Load map
    const map = vectorMaps_1.useVectorMap({
        divRef: mapDiv,
    ...
    vectorMaps_1.useThrottledMapResize(map, props.width, props.height);
    /** Handle a click on a layer */
    const handleLayerClick = useStableCallback_1.useStableCallback((layerViewId, ev) => {
    ...
    const userLayerIds = react_1.useMemo(() => {
        if (!userStyle) {
            return [];
        }
    ...
    // Setup hover effect
    vectorMaps_1.useHoverCursor(map, userLayerIds);
    ...
    return (react_1.default.createElement("div", { style: { width: props.width, height: props.height, position: "relative" } },
        ...
        renderLegend(),
        renderBusy(),
        react_1.default.createElement(vectorMaps_1.AttributionControl, { extraText: props.design.attribution }),
        react_1.default.createElement(vectorMaps_1.MapTilerLogo, null)));
}
