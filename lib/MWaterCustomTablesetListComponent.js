...
exports.MWaterCustomTablesetListComponent = function (props) {
    ...
    var renderTableset = function (ts) {
        ...
        if (search && !name.toLowerCase().includes(search.toLowerCase())
            && !ts.design.tables.some(function (t) { return mwater_expressions_1.ExprUtils.localizeString(t.name).toLowerCase().includes(search.toLowerCase()); })) {
            return null;
        }
        var items = ts.design.tables.filter(function (t) { return !t.deprecated; }).map(function (t) { return ({
