...
class LayeredChartLayerDesignerComponent extends react_1.default.Component {
    constructor() {
        ...
        this.handleCumulativeChange = (ev) => {
            return this.updateLayer({ cumulative: ev.target.checked });
        };
        this.handleTrendlineChange = (ev) => {
    ...
    renderColorAxis() {
        ...
        return R("div", { className: "form-group" }, R("label", { className: "text-muted" }, title), R("div", { style: { marginLeft: 10 } }, R(AxisComponent_1.default, {
            schema: this.props.schema,
            dataSource: this.props.dataSource,
            table: layer.table,
    ...
    renderCumulative() {
        ...
        return R("div", { key: "cumulative" }, R("label", { className: "checkbox-inline" }, R("input", { type: "checkbox", checked: layer.cumulative, onChange: this.handleCumulativeChange }), "Cumulative"));
    }
    renderTrendline() {
        const layer = this.props.design.layers[this.props.index];
        ...
        return R("div", { key: "trendline" }, R("label", { className: "checkbox-inline" }, R("input", { type: "checkbox", checked: layer.trendline === "linear", onChange: this.handleTrendlineChange }), "Show linear trendline"));
    }
    renderStacked() {
        const layer = this.props.design.layers[this.props.index];
        ...
        if (layer.axes.color && this.props.design.layers.length > 1) {
            ...
            return R("div", { key: "stacked" }, R("label", { className: "checkbox-inline" }, R("input", { type: "checkbox", checked: stacked, onChange: this.handleStackedChange }), "Stacked"));
        }
        return null;
    }
    renderColor() {
        ...
        const categoricalX = new LayeredChartCompiler_1.default({ schema: this.props.schema }).isCategoricalX(this.props.design);
        return R("div", { className: "form-group" }, R("label", { className: "text-muted" }, layer.axes.color ? "Default Color" : "Color"), R("div", { style: { marginLeft: 8 } }, R(ColorComponent_1.default, { color: layer.color, onChange: this.handleColorChange }), 
        // Allow toggling of colors
        layer.axes.x && categoricalX && !layer.axes.color
            ? R(bootstrap_1.default.Checkbox, { value: layer.xColorMap, onChange: this.handleXColorMapChange }, "Set Individual Colors")
            : undefined));
    }
    renderFilter() {
        const layer = this.props.design.layers[this.props.index];
