...
describe("PivotChartQueryBuilder", function () {
  ...
  it("creates single query", function () {
    const design = {
      ...
      columns: [{ id: "c1", valueAxis: this.axisText }],
      intersections: {
        "r1:c1": {
  ...
  it("adds filters query", function () {
    const design = {
      table: "t1",
      rows: [{ id: "r1", valueAxis: this.axisEnum, filter: this.exprBoolean }],
      columns: [{ id: "c1", valueAxis: this.axisText }],
      intersections: {
        "r1:c1": {
          valueAxis: this.axisNumberSum
  ...
  it("creates ordered single query", function () {
    const design = {
      table: "t1",
      rows: [{ id: "r1", valueAxis: this.axisEnum, orderExpr: this.exprNumberSum, orderDir: "desc" }],
      columns: [{ id: "c1", valueAxis: this.axisText }],
      intersections: {
        "r1:c1": {
          valueAxis: this.axisNumberSum
    ...
    return compare(query, {
      type: "query",
      selects: [{ type: "select", expr: { type: "field", tableAlias: "main", column: "enum" }, alias: "value" }],
      from: { type: "table", table: "t1", alias: "main" },
      ...
      rows: [
        {
          ...
          orderExpr: this.exprNumberSum,
          orderDir: "desc",
          filter: this.exprBoolean
        }
      ],
      columns: [{ id: "c1", valueAxis: this.axisText }],
      intersections: {
  ...
  return it("creates ordered single query with extraFilters", function () {
    ...
    const queries = this.qb.createQueries(design, [
      { table: "t1", jsonql: { type: "literal", value: false } },
      { table: "t2", jsonql: { type: "literal", value: true } }
    ])
