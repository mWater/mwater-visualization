...
module.exports = TableChart = (function(superClass) {
  ...
  TableChart.prototype.cleanDesign = function(design) {
    ...
    for (j = 0, len = ref1.length; j < len; j++) {
      ordering = ref1[j];
      ordering.axis = this.axisBuilder.cleanAxis(ordering.axis, design.table, "optional");
    }
    if (design.filter) {
  ...
  TableChart.prototype.validateDesign = function(design) {
    ...
    for (j = 0, len1 = ref1.length; j < len1; j++) {
      ...
      error = error || this.axisBuilder.validateAxis(ordering.axis);
    }
  ...
  TableChart.prototype.createQueries = function(design, filters) {
    var colNum, column, expr, exprCompiler, i, j, len, ordering, query, ref, ref1;
