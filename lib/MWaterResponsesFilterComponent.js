var ExprUtils, H, MWaterResponsesFilterComponent, PropTypes, R, React, ui,
...
module.exports = MWaterResponsesFilterComponent = (function(superClass) {
  ...
  MWaterResponsesFilterComponent.prototype.getSiteValue = function() {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      column = ref[i];
      if (column.type === "join" && column.join.type === "n-1" && column.join.toTable.startsWith("entities.")) {
  ...
  MWaterResponsesFilterComponent.prototype.render = function() {
    ...
    siteColumns = _.filter(this.props.schema.getColumns(this.props.table), function(col) {
      return col.type === "join" && col.join.type === "n-1" && col.join.toTable.startsWith("entities.");
    });
    ...
    return H.div({
    ...
    }, "Data Source Options:"), H.div({
      style: {
        paddingLeft: 5
      }
    }, siteColumns.length > 0 ? H.div(null, H.i(null, "This data source contains links to monitoring sites. Would you like to:"), H.div({
      style: {
        paddingLeft: 8
      }
    }, R(ui.Radio, {
      ...
      return function(column) {
        ...
        }, "Show only the latest response for each ", H.i(null, "" + (ExprUtils.localizeString((ref = _this.props.schema.getTable(column.join.toTable)) != null ? ref.name : void 0))), " in the question ", H.i(null, "'" + (ExprUtils.localizeString(column.name)) + "'"));
      };
