...
module.exports = MWaterTableSelectComponent = (function(superClass) {
  ...
  function MWaterTableSelectComponent() {
    this.handleFormChange = bind(this.handleFormChange, this);
  ...
  MWaterTableSelectComponent.prototype.componentWillReceiveProps = function(nextProps) {
    var tableId;
    if (this.state.pendingFormId) {
      tableId = "responses:" + this.state.pendingFormId;
      if (nextProps.schema.getTable(tableId)) {
        this.setState({
          pendingFormId: null
        });
        this.refs.toggleEdit.close();
        return nextProps.onChange(tableId);
      }
    }
  };
  ...
  MWaterTableSelectComponent.prototype.handleFormChange = function(formId) {
    if (indexOf.call(this.props.formIds, formId) < 0) {
      return this.setState({
        pendingFormId: formId
      }, (function(_this) {
        return function() {
          return _this.props.onFormIdsChange([formId].concat(_this.props.formIds));
        };
      })(this));
    } else {
      return this.handleChange("responses:" + formId);
    }
  };
  ...
  MWaterTableSelectComponent.prototype.renderForms = function() {
    return R(FormsListComponent, {
      ...
      includedFormIds: this.props.formIds
    });
  };
    ...
    return $.getJSON(this.props.apiUrl + "forms?" + querystring.stringify(query), (function(_this) {
      return function(forms) {
        forms = _.sortByOrder(forms, [
          function(form) {
            var ref;
            if (ref = form._id, indexOf.call(_this.props.includedFormIds, ref) >= 0) {
              return 1;
            } else {
              return 0;
            }
