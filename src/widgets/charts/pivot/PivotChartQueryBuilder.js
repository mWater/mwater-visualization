      ...
      if (segment.orderExpr) {
        ...
        for (intersectionId of _.keys(design.intersections)) {
          if (intersectionId.includes(segment.id)) { 
            ({
              filter
            } = design.intersections[intersectionId]);
            if (filter) {
        ...
        queries[segment.id] = {
          type: "query",
          selects: [{ type: "select", expr: this.axisBuilder.compileAxis({axis: segment.valueAxis, tableAlias: "main"}), alias: "value" }],
          from: exprCompiler.compileTable(design.table, "main"),
          where,
          groupBy: [1],
          orderBy: [{ expr: exprCompiler.compileExpr({expr: segment.orderExpr, tableAlias: "main"}), direction: segment.orderDir || "asc" }]
        };
      }
    }
