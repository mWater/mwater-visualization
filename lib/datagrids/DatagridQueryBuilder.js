...
module.exports = DatagridQueryBuilder = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(DatagridQueryBuilder, [{
    ...
    value: function createSimpleQuery(design, options) {
      ...
      if (design.filter) {
        expr = exprCleaner.cleanExpr(design.filter, {
          table: design.table
        });
    ...
    value: function createComplexSubtableQuery(design, options, subtable, subtableIndex) {
      ...
      var expr, exprCleaner, exprCompiler, exprUtils, extraFilter, i, j, k, l, len, len1, len2, len3, m, query, ref, ref1, ref2, ref3, selects, st, stindex, subtableTable, types, wheres;
      exprUtils = new ExprUtils(this.schema);
      exprCompiler = new ExprCompiler(this.schema);
      exprCleaner = new ExprCleaner(this.schema); // Create selects
    ...
    value: function getMainOrderByExprs(design) {
      ...
      var exprCleaner, exprCompiler, exprs, j, len, orderBy, orderByExpr, ordering, ref;
      exprCompiler = new ExprCompiler(this.schema);
      exprCleaner = new ExprCleaner(this.schema);
      exprs = [];
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        orderBy = ref[j]; // Clean first

        orderByExpr = exprCleaner.cleanExpr(orderBy.expr, {
          table: design.table
        });
    ...
    value: function getSubtableOrderByExprs(design, subtable) {
      var expr, exprCleaner, exprCompiler, exprUtils, exprs, j, len, orderBy, ordering, ref, subtableTable;
      exprUtils = new ExprUtils(this.schema);
      exprCompiler = new ExprCompiler(this.schema);
      exprCleaner = new ExprCleaner(this.schema); // Get subtable actual table
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        orderBy = ref[j];
        expr = exprCleaner.cleanExpr(orderBy.expr, {
          table: subtableTable
        });
        exprs.push(exprCompiler.compileExpr({
          expr: expr,
