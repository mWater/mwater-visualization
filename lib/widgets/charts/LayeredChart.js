var AxisBuilder, Chart, ExpressionBuilder, H, LayeredChart, LayeredChartCompiler, LayeredChartDesignerComponent, LayeredChartSvgFileSaver, LayeredChartUtils, LayeredChartViewComponent, React, _,
...
module.exports = LayeredChart = (function(superClass) {
  ...
  LayeredChart.prototype.createDataTable = function(design, data) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      r = [];
      if (design.layers[0].axes.x) {
        r.push(this.axisBuilder.formatValue(design.layers[0].axes.x, row.x));
      }
      if (design.layers[0].axes.color) {
        r.push(this.axisBuilder.formatValue(design.layers[0].axes.color, row.color));
      }
      if (design.layers[0].axes.y) {
        r.push(this.axisBuilder.formatValue(design.layers[0].axes.y, row.y));
      }
      table.push(r);
    }
    return table;
