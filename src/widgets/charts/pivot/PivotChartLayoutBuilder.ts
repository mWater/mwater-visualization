...
export default PivotChartLayoutBuilder = class PivotChartLayoutBuilder {
      ...
      if (_.any(needsSpecialRowHeader)) {
        ...
        for (column of columns) {
          ...
          const intersectionId = PivotChartUtils.getIntersectionId(
            _.map(row, (r) => r.segment),
            _.map(column, (c) => c.segment)
          )
  ...
  setupBorders(layout) {
    ...
    for (
      ...
      var asc1, end1
      for (
        rowIndex = 0, end1 = layout.rows.length, asc1 = 0 <= end1;
        asc1 ? rowIndex < end1 : rowIndex > end1;
        asc1 ? rowIndex++ : rowIndex--
      ) {
        cell = layout.rows[rowIndex].cells[columnIndex]
        ...
        if (cell.type === "column") {
          ...
          if (cell.sectionRight) {
            ...
            columnIndex < layout.rows[rowIndex].cells.length - 1 &&
  ...
  getRowsOrColumns(isRow, segment, data, locale, parentSegments = [], parentValues = []) {
    ...
    for (let category of categories) {
      for (let childSegment of segment.children) {
        // Get child results
        const childResults = this.getRowsOrColumns(
          isRow,
          childSegment,
          data,
          locale,
          parentSegments.concat([segment]),
          parentValues.concat([category.value])
        )
