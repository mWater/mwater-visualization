...
function NewMapViewComponent(props) {
    ...
    var designRef = react_2.useRef();
    /** Layers present */
    var currentLayersRef = react_2.useRef([]);
    /** Sources present */
    var currentSourcesRef = react_2.useRef([]);
    /** Busy incrementable counter. Is busy if > 0 */
    var _b = react_1.useState(0), busy = _b[0], setBusy = _b[1];
    /** Layer click handlers present */
    var currentLayerClickHandlers = react_2.useRef([]);
    /** True when symbols have been added to map */
    var _c = react_1.useState(false), symbolsAdded = _c[0], setSymbolsAdded = _c[1];
    /** Base layer id currently loaded */
    var _d = react_1.useState(""), baseLayerLoaded = _d[0], setBaseLayerLoaded = _d[1];
    /** Contents of popup if open */
    var _e = react_1.useState(), popupContents = _e[0], setPopupContents = _e[1];
    /** Datetime layers must have been created after on server. Used to support refreshing */
    var _f = react_1.useState(new Date().toISOString()), layersCreatedAfter = _f[0], setLayersCreatedAfter = _f[1];
    ...
    var _g = react_1.useState(initialLegendDisplay == "closed" || (props.width < 500 && initialLegendDisplay == "closedIfSmall")), legendHidden = _g[0], setLegendHidden = _g[1];
    // Store design in ref to detect changes
    react_1.useEffect(function () { designRef.current = props.design; }, [props.design]);
    ...
    function updateLayers() {
        return __awaiter(this, void 0, void 0, function () {
            ...
            var earliestExpires, compiledFilters, scopedCompiledFilters, newSources, newLayers, newClickHandlers, _i, _a, layerView, isScoping, _b, _c, currentLayer, _d, _e, source, _f, _g, currentLayerClickHandler, _h, newSources_1, source, _j, newLayers_1, newLayer, _loop_1, _k, newClickHandlers_1, newClickHandler;
            return __generator(this, function (_l) {
                switch (_l.label) {
                    ...
                    case 6:
                        ...
                        for (_b = 0, _c = currentLayersRef.current; _b < _c.length; _b++) {
                            currentLayer = _c[_b];
                            map.removeLayer(currentLayer.id);
                        }
                        currentLayersRef.current = [];
                        for (_d = 0, _e = currentSourcesRef.current; _d < _e.length; _d++) {
                            source = _e[_d];
                            map.removeSource(source.id);
                        }
                        currentSourcesRef.current = [];
                        // Remove click handlers
                        for (_f = 0, _g = currentLayerClickHandlers.current; _f < _g.length; _f++) {
                            currentLayerClickHandler = _g[_f];
                        ...
                        for (_h = 0, newSources_1 = newSources; _h < newSources_1.length; _h++) {
                            source = newSources_1[_h];
                            map.addSource(source.id, source.sourceData);
                            currentSourcesRef.current.push(source);
                        }
                        for (_j = 0, newLayers_1 = newLayers; _j < newLayers_1.length; _j++) {
                            newLayer = newLayers_1[_j];
                            map.addLayer(newLayer.layer);
                            currentLayersRef.current.push(newLayer.layer);
                        }
                        _loop_1 = function (newClickHandler) {
                            ...
                            currentLayerClickHandlers.current.push({ layerId: newClickHandler.mapLayerId, clickHandler: clickHandler });
                        };
                        for (_k = 0, newClickHandlers_1 = newClickHandlers; _k < newClickHandlers_1.length; _k++) {
                            newClickHandler = newClickHandlers_1[_k];
                            _loop_1(newClickHandler);
                        }
                        return [3 /*break*/, 8];
                    case 7:
                        setBusy(function (b) { return b - 1; });
    ...
    react_1.useEffect(function () {
        ...
        m.addControl(new mapbox_gl_1.default.NavigationControl({ showCompass: false }), "top-left");
    ...
    react_1.useEffect(function () {
        ...
        addSymbolsToMap(map).then(function () {
...
function addSymbolsToMap(map) {
    var promises = [];
    var _loop_2 = function (mapSymbol) {
        promises.push(new Promise(function (resolve, reject) {
            map.loadImage(mapSymbol.url, function (err, image) {
                if (err) {
                    reject(err);
                }
                else {
    ...
    for (var _i = 0, mapSymbols_2 = mapSymbols_1.mapSymbols; _i < mapSymbols_2.length; _i++) {
        var mapSymbol = mapSymbols_2[_i];
        _loop_2(mapSymbol);
    }
