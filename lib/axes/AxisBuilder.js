    ...
    */
    ...
    getCategories(axis, values, options = {}) {
        ...
        if (axis.xform && axis.xform.type === "yearmonth") {
            ...
            if (options.onlyValuesPresent) {
                // Sort and take only present
                categories = lodash_1.default.sortBy(lodash_1.default.uniq(values), item => item).map(value => ({ value, label: moment_1.default(value, "YYYY-MM-DD").format("MMM YYYY") }));
                if (hasNone) {
                    categories.push(noneCategory);
                }
                return categories;
        ...
        switch (this.getAxisType(axis)) {
            ...
            case "date":
                ...
                if (options.onlyValuesPresent) {
                    // Sort and take only present
                    categories = lodash_1.default.sortBy(lodash_1.default.uniq(values), item => item).map(value => ({ value, label: moment_1.default(value, "YYYY-MM-DD").format("ll") }));
                    if (hasNone) {
                        categories.push(noneCategory);
                    }
                    return categories;
