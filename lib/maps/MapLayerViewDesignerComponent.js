...
class MapLayerViewDesignerComponent extends react_1.default.Component {
    ...
    renderAdvanced() {
        ...
        }, R(bootstrap_1.default.Checkbox, { value: this.props.layerView.hideLegend, onChange: this.handleHideLegend, inline: true }, "Hide Legend"), R("label", { className: "text-muted", key: "label" }, "Group:"), R(bootstrap_1.default.TextInput, {
            key: "input",
    ...
    renderName() {
        return R("span", { className: "hover-display-parent", onClick: this.handleRename, style: { cursor: "pointer" } }, this.props.layerView.name, " ", R("span", { className: "hover-display-child glyphicon glyphicon-pencil text-muted" }));
    }
    renderEditor() {
        const layer = LayerFactory_1.default.createLayer(this.props.layerView.type);
    ...
    renderLayerEditToggle() {
        return R("div", { key: "edit", style: { marginBottom: this.state.editing ? 10 : undefined } }, R("a", { onClick: this.handleToggleEditing, style: { fontSize: 12, cursor: "pointer" } }, this.state.editing
            ? [R("i", { className: "fa fa-caret-up" }), " Close"]
            : [R("i", { className: "fa fa-cog" }), " Customize..."]));
    }
    renderOpacityControl() {
        return R("div", { className: "form-group", style: { paddingTop: 10 } }, R("label", { className: "text-muted" }, R("span", null, `Opacity: ${Math.round(this.props.layerView.opacity * 100)}%`)), R("div", { style: { padding: "10px" } }, react_1.default.createElement(rc_slider_1.default, {
            min: 0,
            max: 100,
            step: 1,
    ...
    renderDeleteLayer() {
        return R("div", { style: { float: "right", cursor: "pointer", marginLeft: 10 }, key: "delete" }, R("a", { onClick: this.handleRemove }, R("i", { className: "fa fa-remove" })));
    }
    render() {
        const layer = LayerFactory_1.default.createLayer(this.props.layerView.type);
        const style = {
