...
exports.default = AxisBuilder = class AxisBuilder {
    // Options are: schema
    constructor(options) {
    ...
    getCategories(axis, values, locale) {
        ...
        if (axis.xform && axis.xform.type === "floor") {
            ...
            const hasNull = lodash_1.default.filter(values, (v) => v == null).length > 0;
        ...
        if (axis.xform && axis.xform.type === "month") {
            ...
            // Add none if needed
            if (lodash_1.default.any(values, (v) => v == null)) {
                categories.push(noneCategory);
            }
            return categories;
        }
        ...
        if (axis.xform && axis.xform.type === "year") {
            let asc1, end2;
            hasNone = lodash_1.default.any(values, (v) => v == null);
        ...
        switch (this.getAxisType(axis)) {
            ...
            case "enumset":
                ...
                return lodash_1.default.map(this.exprUtils.getExprEnumValues(axis.expr), (ev) => ({
                    value: ev.id,
                    label: mwater_expressions_3.ExprUtils.localizeString(ev.name, locale)
                })).concat([noneCategory]);
                break;
            case "text":
    ...
    doesAxisSupportCumulative(axis) {
        var _a;
        let needle;
        return (((needle = this.getAxisType(axis)), ["date", "number"].includes(needle)) || ((_a = axis.xform) === null || _a === void 0 ? void 0 : _a.type) === "yearquarter");
    }
