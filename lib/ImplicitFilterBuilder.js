...
class ImplicitFilterBuilder {
    ...
    findJoins(filterableTables) {
        ...
        for (var filterableTable of filterableTables) {
            ...
            let joins = lodash_1.default.filter(this.schema.getColumns(filterableTable), (column) => (column.type === "join" && column.join.type === "n-1" && column.join.toTable !== filterableTable) || (column.type === "id" && column.idTable !== filterableTable));
            // Only keep if singular
            joins = lodash_1.default.flatten(lodash_1.default.filter(lodash_1.default.values(lodash_1.default.groupBy(joins, (join) => join.type === "id" ? join.idTable : join.join.toTable)), (list) => list.length === 1));
    ...
    extendFilters(filterableTables, filters) {
        ...
        for (var join of joins) {
            const parentFilters = lodash_1.default.filter(filters, (f) => {
