...
module.exports = BufferLayer = (function(superClass) {
  ...
  BufferLayer.prototype.onGridClick = function(ev, clickOptions) {
    ...
    if (ev.data && ev.data.id) {
      ...
      if (clickOptions.design.popup && !ev.event.originalEvent.ctrlKey) {
      ...
      } else if (!ev.event.originalEvent.ctrlKey) {
        results.row = {
          tableId: table,
  ...
  BufferLayer.prototype.createKMLExportJsonQL = function(design, schema, filters) {
    var axisBuilder, bufferedGeometry, cases, categories, colorExpr, exprCompiler, filter, geometryExpr, j, len, order, query, radiusDeg, relevantFilters, selects, whereClauses;
    axisBuilder = new AxisBuilder({
      schema: schema
    });
    exprCompiler = new ExprCompiler(schema);
