...
module.exports = MarkersLayer = (function(superClass) {
  ...
  MarkersLayer.prototype.getTileUrl = function(filters) {
    if (!this.design.axes || !this.design.axes.geometry) {
      return null;
    }
  ...
  MarkersLayer.prototype.createUrl = function(extension, filters) {
    var mapDesign, query;
    query = "type=jsonql";
    if (this.client) {
    ...
    mapDesign = {
      layers: [
        {
          id: "layer0",
          jsonql: this.createJsonQL(filters)
        }
      ],
      css: this.createCss()
    };
  ...
  MarkersLayer.prototype.createJsonQL = function(filters) {
    ...
    geometryExpr = axisBuilder.compileAxis({
      axis: this.design.axes.geometry,
    ...
    innerquery = {
      ...
      from: exprCompiler.compileTable(this.design.table, "innerquery")
    };
    whereClauses = [
      {
        type: "op",
    ...
    if (this.design.filter) {
      whereClauses.push(exprCompiler.compileExpr({
        expr: this.design.filter,
        tableAlias: "innerquery"
      }));
    }
