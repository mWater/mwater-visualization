...
class TableChart extends Chart_1.default {
        ...
        design = (0, immer_1.default)(design, (draft) => {
            ...
            for (let columnId = 0, end = draft.columns.length, asc = 0 <= end; asc ? columnId < end : columnId > end; asc ? columnId++ : columnId--) {
                ...
                column.backgroundColorAxis = axisBuilder.cleanAxis({
                    axis: column.backgroundColorAxis ? (0, immer_2.original)(column.backgroundColorAxis) : null,
                    table: design.table,
                    aggrNeed: "optional"
                });
            }
    ...
    getData(design, schema, dataSource, filters, callback) {
        ...
        const isAggr = lodash_1.default.any(design.columns, (column) => axisBuilder.isAxisAggr(column.textAxis)) ||
            ...
            lodash_1.default.any(design.columns, (column) => axisBuilder.isAxisAggr(column.backgroundColorAxis));
        const totalColumns = [];
        // For each column
        ...
        const summaryQuery = {
            ...
            selects: design.columns.map((c, i) => {
                ...
                if (totalColumns.includes(`c${i}`))
                    return { type: "select", expr: { type: "op", op: (_a = c.summaryType) !== null && _a !== void 0 ? _a : 'sum', exprs: [{ type: "field", tableAlias: "t", column: `c${i}` }] }, alias: `c${i}` };
                else
