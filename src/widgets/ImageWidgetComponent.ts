  ...
  ImageWidgetComponent = class ImageWidgetComponent extends AsyncLoadComponent {
    ...
    renderContent() {
      if (this.props.design.imageUrl || this.props.design.uid) {
        ...
        return R(RotatedImageComponent, {imgUrl: source, url: this.props.design.url, rotation: this.props.design.rotation});
      } else {
      ...
      return R(DropdownWidgetComponent, { 
        ...
        dropdownItems
          ...
          !this.props.design.imageUrl && !this.props.design.expr && !this.props.design.uid && this.props.onDesignChange ?
            this.renderEditLink()
          :
            R('div', {className: "mwater-visualization-image-widget", style: { position: "relative", width: this.props.width, height: this.props.height}},
              captionPosition === "top" ?
                R('div', {className: "caption"}, this.props.design.caption) : undefined,
...
class ImageWidgetDesignComponent extends React.Component {
  ...
  setCurrentTab() {
    ...
    if (this.props.design.url) { tab = "url"; }
    if (this.props.design.expr) { tab = "expression"; }
  ...
  renderRotation() {
    return R('div', {style: { paddingTop: 10 }}, 
      "Rotation: ",
      R(ui.Radio, {value: this.state.rotation || null, radioValue: null, onChange: this.handleRotationChange, inline: true}, "0 degrees"),
      R(ui.Radio, {value: this.state.rotation || null, radioValue: 90, onChange: this.handleRotationChange, inline: true}, "90 degrees"),
      R(ui.Radio, {value: this.state.rotation || null, radioValue: 180, onChange: this.handleRotationChange, inline: true}, "180 degrees"),
      R(ui.Radio, {value: this.state.rotation || null, radioValue: 270, onChange: this.handleRotationChange, inline: true}, "270 degrees"));
  }

  renderImageUrlEditor() {
    return R('div', {className: "form-group"},
      R('label', null, "URL of image"),
      R('input', {type: "text", className: "form-control", value: this.state.imageUrl || "", onChange: this.handleImageUrlChange}),
  ...
  renderUrlEditor() {
    return R('div', {className: "form-group"},
      ...
      R('input', {type: "text", className: "form-control", value: this.state.url || "", onChange: this.handleUrlChange}),
      R('p', {className: "help-block"},
  ...
  render() {
    ...
    const content = R('div', null,
      R('div', {className: "form-group"},
        R('label', null, "Caption"),
        R('input', {type: "text", className: "form-control", value: this.state.caption || "", onChange: this.handleCaptionChange, placeholder: "Optional caption to display below image"})),
      ...
      this.state.imageUrl || this.state.uid ?
        this.renderUrlEditor() : undefined
    );
