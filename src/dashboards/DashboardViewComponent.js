  ...
  DashboardViewComponent = class DashboardViewComponent extends React.Component {
    ...
    handleClipboardChange(block) { 
      try {
        ...
        const tables = _.pluck(_.filter(this.props.schema.getTables(), table => JSON.stringify(block).includes(JSON.stringify(table.id))), "id");
    ...
    render() {
      ...
      const renderWidget = options => {
        ...
        if (this.props.design.implicitFiltersEnabled || (this.props.design.implicitFiltersEnabled == null)) { // Default is true
      ...
      return R("div", {style}, 
        !this.props.hideScopes ?
          this.renderScopes() : undefined,
