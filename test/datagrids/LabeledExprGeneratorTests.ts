...
describe("LabeledExprGenerator", function () {
      ...
      if (column.id[0] === "c") {
        expectedLes.push({
          expr: {
            ...
            type: "scalar",
            joins: ["2-1"],
            expr: { type: "field", table: "custom.ts15.t0", column: column.id }
          },
          label: `T2->T1 > ${column.name.en}`,
          joins: []
        })
      }
    }
  ...
  it("includes 1-n joins", function () {
    ...
    for (column of this.schema.getColumns("t2")) {
      if (column.type !== "join") {
        expectedLes.push({
          expr: { type: "field", table: "t2", column: column.id },
          label: column.name.en,
          joins: ["1-2"]
        })
      }
    }
