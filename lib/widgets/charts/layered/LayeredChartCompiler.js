...
pieLabelValueFormatter = (function(_this) {
  return function(value, ratio, id) {
    return (d3Format.format(",")(value)) + " (" + (d3Format.format('.1%')(ratio)) + ")";
  };
})(this);
...
module.exports = LayeredChartCompiler = (function() {
  ...
  LayeredChartCompiler.prototype.compileDataNonCategorical = function(design, data, locale) {
    ...
    _.each(design.layers, (function(_this) {
      return function(layer, layerIndex) {
        var categories, categoryOrder, colorValues, layerData, ref, ref1, seriesX, seriesY, yValues;
        layerData = data["layer" + layerIndex];
        ...
        if (layer.axes.color) {
          ...
          return _.each(colorValues, function(colorValue) {
            var color, ref, ref1, rows, seriesX, seriesY, yValues;
      ...
      return function(layer, layerIndex) {
        ...
        if (layer.axes.color) {
          ...
          _.each(layerData, function(row) {
            ...
            index = categoryMap[row.x];
            column[index] = row.y;
            dataMap[series + ":" + index] = {
              layerIndex: layerIndex,
              row: row
            };
            if (layer.xColorMap) {
              color = _this.axisBuilder.getValueColor(layer.axes.x, row.x);
              if (color) {
    ...
    return {
      ...
      color: (function(_this) {
        return function(color, d) {
          var key, len3, p, sortedYThresholds, yThreshold;
          if (typeof d !== "object") {
            return color;
          }
          key = d.id + ":" + d.index;
