var ExprCleaner, ExprCompiler, R, React, TextWidget, Widget, _, async, injectTableAlias,
...
module.exports = TextWidget = (function(superClass) {
  ...
  TextWidget.prototype.getTOCEntries = function(design, namedStrings) {
    var entries, findRecursive, flattenText;
    entries = [];
    flattenText = function(items) {
      var text;
      ...
      return text = text.replace(/\{\{.+?\}\}/g, (function(_this) {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        ...
        if ((item != null ? item.type : void 0) === "element" && item.tag.match(/^h[1-9]$/)) {
          entries.push({
            ...
            text: flattenText(item.items)
          });
        }
        if (item != null ? item.items : void 0) {
          results.push(findRecursive(item.items));
        } else {
          results.push(void 0);
        }
      }
