    ...
    ChoroplethLayer.prototype.createJsonQL = function (design, schema, filters) {
        ...
        if (design.regionMode === "indirect" || !design.regionMode) {
            ...
            */
            ...
            for (var _a = 0, relevantFilters_1 = relevantFilters; _a < relevantFilters_1.length; _a++) {
                var filter = relevantFilters_1[_a];
            ...
            // Add filters on regions to outer query
            for (var _b = 0, filters_2 = filters; _b < filters_2.length; _b++) {
                var filter = filters_2[_b];
                if (filter.table == regionsTable) {
        ...
        if (design.regionMode === "direct") {
            ...
            */
            ...
            // Add filters on regions to outer query
            for (var _c = 0, filters_3 = filters; _c < filters_3.length; _c++) {
                var filter = filters_3[_c];
                if (filter.table == regionsTable) {
                    query.where.exprs.push(mwater_expressions_1.injectTableAlias(filter.jsonql, "regions"));
                }
            }
