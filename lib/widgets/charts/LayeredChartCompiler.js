var AxisBuilder, ExpressionBuilder, ExpressionCompiler, LayeredChartCompiler, colorHacks, injectTableAlias;
...
module.exports = LayeredChartCompiler = (function() {
    ...
    chartDesign = {
      ...
      axis: {
        x: {
          ...
          label: {
            text: options.design.xAxisLabelText,
            position: 'outer-center'
          }
        },
        y: {
          label: {
            text: options.design.yAxisLabelText,
            position: 'outer-center'
          },
      ...
      return function(layer, layerIndex) {
        ...
        if (layer.axes.color) {
          return _.each(data["layer" + layerIndex], function(row, rowIndex) {
            ...
            names[series] = _this.formatAxisValue(layer.axes.color, row.color);
            dataMap[series] = {
      ...
      return function(layer, layerIndex) {
        ...
        if (layer.axes.color) {
          ...
          return _.each(colorValues, function(colorValue) {
            ...
            names[seriesY] = _this.formatAxisValue(layer.axes.color, colorValue);
            xs[seriesY] = seriesX;
  ...
  LayeredChartCompiler.prototype.compileDataCategorical = function(design, data) {
    var categories, categoryMap, colors, column, columns, dataMap, groups, i, j, k, len, len1, m, n, names, ref, ref1, types, xAxis, xValues, xs, xtotals;
    columns = [];
    types = {};
    names = {};
    dataMap = {};
      ...
      return function(layer, layerIndex) {
        ...
        if (layer.axes.color) {
          ...
          return _.each(colorValues, function(colorValue) {
            ...
            types[series] = _this.getLayerType(design, layerIndex);
            names[series] = _this.formatAxisValue(layer.axes.color, colorValue);
            return xs[series] = "x";
          });
        } else {
          series = "" + layerIndex;
  ...
  LayeredChartCompiler.prototype.formatAxisValue = function(axis, value) {
    var item, items;
    if (value && this.exprBuilder.getExprType(axis.expr) === "enum") {
      items = this.exprBuilder.getExprValues(axis.expr);
  ...
  LayeredChartCompiler.prototype.createScope = function(design, layerIndex, row) {
    ...
    if (layer.axes.x) {
      ...
      names.push(this.axisBuilder.summarizeAxis(layer.axes.x) + " is " + this.axisBuilder.stringifyLiteral(layer.axes.x, row.x));
      data.x = row.x;
    }
    if (layer.axes.color) {
      filters.push(this.axisBuilder.createValueFilter(layer.axes.color, row.color));
      names.push(this.axisBuilder.summarizeAxis(layer.axes.color) + " is " + this.axisBuilder.stringifyLiteral(layer.axes.color, row.color));
