...
function dropBlock(rootBlock, sourceBlock, targetBlock, side) {
    ...
    if (["vertical", "root"].includes(rootBlock.type)) {
        ...
        ({ blocks } = rootBlock);
        // Find target block
        index = lodash_1.default.findIndex(blocks, { id: targetBlock.id });
        if (index >= 0) {
            blocks = blocks.slice();
            // Add
            switch (side) {
            ...
            // Recurse
            blocks = lodash_1.default.map(blocks, (block) => exports.dropBlock(block, sourceBlock, targetBlock, side));
            return lodash_1.default.extend({}, rootBlock, { blocks });
        }
    }
    // If horizontal
...
function updateBlock(rootBlock, block) {
    ...
    if (["vertical", "horizontal", "root"].includes(rootBlock.type)) {
        ...
        // Recurse
        blocks = lodash_1.default.map(blocks, (b) => exports.updateBlock(b, block));
        return lodash_1.default.extend({}, rootBlock, { blocks });
    }
    return rootBlock;
}
...
function removeBlock(rootBlock, block) {
    ...
    if (["vertical", "horizontal", "root"].includes(rootBlock.type)) {
        ...
        // TODO remove weight
        // Recurse
        blocks = lodash_1.default.compact(lodash_1.default.map(blocks, (b) => exports.removeBlock(b, block)));
...
function cleanBlock(rootBlock) {
    ...
    if (["vertical", "horizontal", "root"].includes(rootBlock.type)) {
        ...
        // Recurse
        blocks = lodash_1.default.map(blocks, (b) => exports.cleanBlock(b));
        return lodash_1.default.extend({}, rootBlock, { blocks });
    }
    return rootBlock;
}
