...
class AxisComponent extends AsyncLoadComponent_1.default {
    constructor(props) {
        ...
        this.handleFormatChange = (ev) => {
            return this.props.onChange(lodash_1.default.extend({}, this.props.value, { format: ev.target.value }));
        };
        this.handleXformTypeChange = (type) => {
            ...
            let xform;
            if (!type) {
                this.props.onChange(lodash_1.default.omit(this.props.value, ["xform", "colorMap", "drawOrder"]));
            }
            ...
            if (type === "ranges" &&
                ...
                const ranges = [{ id: uuid_1.default(), maxValue: min, minOpen: false, maxOpen: true }];
                for (let i = 1, end1 = numBins, asc = 1 <= end1; asc ? i <= end1 : i >= end1; asc ? i++ : i--) {
                    ...
                    ranges.push({ id: uuid_1.default(), minValue: start, minOpen: false, maxValue: end, maxOpen: true });
                }
                ranges.push({ id: uuid_1.default(), minValue: max, minOpen: true, maxOpen: true });
                xform = {
                    type: "ranges",
                    ranges
                };
            ...
            return this.props.onChange(update_object_1.default(lodash_1.default.omit(this.props.value, ["colorMap", "drawOrder"]), { xform: { $set: xform } }));
        };
        this.handleXformChange = (xform) => {
            return this.props.onChange(this.cleanAxis(update_object_1.default(lodash_1.default.omit(this.props.value, ["drawOrder"]), { xform: { $set: xform } })));
        };
        this.state = {
    ...
    load(props, prevProps, callback) {
        ...
        let whereClauses = lodash_1.default.map(filters, (f) => mwater_expressions_2.injectTableAlias(f.jsonql, "main"));
    ...
    renderFormat(axis) {
        ...
        const formats = valueFormatter_1.getFormatOptions(valueType);
        if (!formats) {
            return null;
        }
        return R("div", { className: "form-group" }, R("label", { className: "text-muted" }, "Format"), ": ", R("select", {
            value: axis.format != null ? axis.format : valueFormatter_2.getDefaultFormat(valueType),
