var AxisBuilder, ExprCleaner, ExprCompiler, ExprUtils, H, Layer, LegendGroup, LegendItem, MarkersLayer, React, _, injectTableAlias,
...
module.exports = MarkersLayer = (function(superClass) {
  ...
  MarkersLayer.prototype.getLegend = function(design, schema, name) {
    ...
    items = _.map(design.sublayers, (function(_this) {
      return function(sublayer, i) {
        var colors, enums, legendGroupProps, symbol, title;
        title = sublayer.name ? sublayer.name : ExprUtils.localizeString(schema.getTable(sublayer.axes.geometry.expr.table).name);
        if (sublayer.axes.color && sublayer.axes.color.colorMap) {
        ...
        symbol = sublayer.symbol ? sublayer.symbol : 'font-awesome/circle';
        legendGroupProps = {
          symbol: symbol,
          items: colors,
