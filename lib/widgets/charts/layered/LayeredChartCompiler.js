...
var AxisBuilder, ExprCompiler, ExprUtils, LayeredChartCompiler, _, cleanString, d3Format, d3Formatter, injectTableAlias, labelValueFormatter, pieLabelValueFormatter, tickFormatter;
...
pieLabelValueFormatter = function pieLabelValueFormatter(value, ratio, id) {
  return "".concat(d3Format.format(",")(value), " (").concat(d3Format.format('.1%')(ratio), ")");
};

d3Formatter = function d3Formatter(format) {
...
module.exports = LayeredChartCompiler =
/*#__PURE__*/
function () {
  // Pass in schema
  ...
  (0, _createClass2.default)(LayeredChartCompiler, [{
    ...
    value: function createChartOptions(options) {
      ...
      c3Data = this.compileData(options.design, options.data, options.locale); // Create chart
      ...
      _.each(design.layers, function (layer, layerIndex) {
        ...
        if (layer.axes.color) {
          ...
          if (row) {
            ...
            names[series] = layer.name || "Series ".concat(layerIndex + 1);
            dataMap[series] = {
              layerIndex: layerIndex,
              row: row
    ...
    value: function compileDataNonCategorical(design, data, locale) {
      ...
      _.each(design.layers, function (layer, layerIndex) {
        var categories, categoryOrder, colorValues, layerData, ref, ref1, seriesX, seriesY, yValues; // Get data of layer
        ...
        if (layer.axes.color) {
          ...
          return _.each(colorValues, function (colorValue) {
            var color, ref, ref1, rows, seriesX, seriesY, yValues; // One series for x values, one for y
            ...
            names[seriesY] = _this2.axisBuilder.formatValue(layer.axes.color, colorValue, locale);
            xs[seriesY] = seriesX;
          ...
          types[seriesY] = _this2.getLayerType(design, layerIndex);
          names[seriesY] = layer.name || "Series ".concat(layerIndex + 1);
          xs[seriesY] = seriesX;
    ...
    value: function compileDataCategorical(design, data, locale) {
      ...
      columns.push(["x"].concat(_.pluck(categories, "label"))); // For each layer

      _.each(design.layers, function (layer, layerIndex) {
        var colorCategories, colorCategoryOrder, colorValues, column, layerData, ref, ref1, series; // Get data of layer
        ...
        if (layer.axes.color) {
          ...
          return _.each(colorValues, function (colorValue) {
            var color, column, i, j, len, ref, ref1, rows, series, value; // One series for y values
            ...
            names[series] = _this3.axisBuilder.formatValue(layer.axes.color, colorValue, locale);
            xs[series] = "x";
    ...
    value: function makeRowsCumulative(rows) {
      ...
      for (j = 0, len = rows.length; j < len; j++) {
        ...
        y = total + row.y;
        totals[row.color] = y; // Create new row
