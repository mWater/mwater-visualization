...
module.exports = LayeredChartCompiler = (function() {
  ...
  LayeredChartCompiler.prototype.compileExpr = function(expr, aggr) {
    var exprCompiler;
  ...
  LayeredChartCompiler.prototype.getQueries = function(design, extraFilters) {
    var colorExpr, exprCompiler, filter, i, j, layer, layerIndex, len, queries, query, ref, relevantFilters, whereClauses, xExpr, yExpr;
    exprCompiler = new ExpressionCompiler(this.schema);
    queries = {};
    for (layerIndex = i = 0, ref = design.layers.length; 0 <= ref ? i < ref : i > ref; layerIndex = 0 <= ref ? ++i : --i) {
      ...
      yExpr = this.compileExpr(layer.yExpr, layer.yAggr);
      if (xExpr) {
        query.selects.push({
          type: "select",
          expr: xExpr,
