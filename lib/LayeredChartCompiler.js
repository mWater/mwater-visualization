...
module.exports = LayeredChartCompiler = (function() {
  ...
  LayeredChartCompiler.prototype.getQueries = function(design, extraFilters) {
    var filter, i, j, layer, layerId, len, queries, query, ref, relevantFilters, whereClauses, yExpr;
    queries = {};
    for (layerId = i = 0, ref = design.layers.length; 0 <= ref ? i < ref : i > ref; layerId = 0 <= ref ? ++i : --i) {
      ...
      query.selects.push({
        type: "select",
        expr: this.compileExpr(layer.xExpr),
        alias: "x"
      });
      if (layer.colorExpr) {
        query.selects.push({
          type: "select",
          expr: this.compileExpr(layer.colorExpr),
          alias: "color"
        });
      }
      query.orderBy.push({
      ...
      if (this.doesLayerNeedGrouping(design, layerId)) {
        ...
        query.selects.push({
          type: "select",
          expr: this.compileExpr(layer.yExpr),
          alias: "y"
        });
      }
      if (layer.filter) {
  ...
  LayeredChartCompiler.prototype.getColumns = function(design, data) {
    var colorVal, colorValues, columns, i, j, k, l, layer, layerId, len, len1, len2, len3, len4, len5, m, n, o, p, ref, ref1, ref2, ref3, row, val, xCategorical, xValues, xcolumn, ycolumn;
    columns = [];
