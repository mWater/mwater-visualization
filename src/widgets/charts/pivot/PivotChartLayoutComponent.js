      ...
      return R('tr', {key: rowIndex},
        _.map(row.cells, (cell, columnIndex) => {
          return R(LayoutCellComponent, {
            ...
            onSummarizeSegment: this.props.onSummarizeSegment ? this.props.onSummarizeSegment.bind(null, cell.section) : undefined
          }
          );
        })
      );
...
`
          ...
          this.props.layout.tooManyRows ?
            R('div', {className: "text-warning", style: { fontSize: 12 }},
              ...
              " Warning: Too many rows in table to display") : undefined,

          this.props.layout.tooManyColumns ?
            R('div', {className: "text-warning", style: { fontSize: 12 }},
              R('i', {className: "fa fa-exclamation-circle"}),
              " Warning: Too many columns in table to display") : undefined,
...
class LayoutCellComponent extends React.Component {
  ...
  renderUnconfigured(cell) {
    return R('span', {style: { fontSize: "90%" }},
      R('a', {style: { cursor: "pointer" }, onClick: this.props.onEditSection},
        "Edit"),
      cell.summarize ?
        [
          R('span', {className: "text-muted"},
            " / "),
          R('a', {style: { cursor: "pointer" }, onClick: this.props.onSummarizeSegment},
            "Summarize")
        ] : undefined);
  }
  ...
  render() {
    ...
    let backgroundColor = cell.unconfigured && this.props.onEditSection ?
      "#eff5fb"
    :
    ...
    // Add striping
    if ((this.props.layout.striping === "columns") && ['column', 'intersection'].includes(cell.type) && ((this.props.columnIndex % 2) === 0)) {
      backgroundColor = Color(backgroundColor || "#ffffff").darken(0.03);
    } else if ((this.props.layout.striping === "rows") && ['row', 'intersection'].includes(cell.type) && ((this.props.rowIndex % 2) === 0)) {
      backgroundColor = Color(backgroundColor || "#ffffff").darken(0.03);
    }
    ...
    const borderBottom = Math.max(cell.borderBottom || 0, this.props.layout.rows[this.props.rowIndex + 1]?.cells[this.props.columnIndex].borderTop || 0);
    const borderRight = Math.max(cell.borderRight || 0, this.props.layout.rows[this.props.rowIndex].cells[this.props.columnIndex + 1]?.borderLeft || 0);
    ...
    return R('td', { 
      ref: (c => { return this.tdComponent = c; }),
