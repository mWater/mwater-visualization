var BlocksLayoutManager, DirectLayerDataSource, DirectMapDataSource, DirectWidgetDataSource, LayerFactory, MapBoundsCalculator, MapDataSource, WidgetFactory, _, injectTableAlias, querystring,
...
module.exports = DirectMapDataSource = (function(superClass) {
  ...
  DirectMapDataSource.prototype.getPrintUrl = function(design, scale) {
    if (!this.options.mapId) {
      return null;
    }
    return this.options.apiUrl + ("maps/" + this.options.mapId + "/print?") + querystring.stringify({
      client: this.options.client,
      scale: scale,
      bounds: JSON.stringify(design.bounds),
      layers: _.pluck(_.where(design.layerViews, {
        visible: true
      }), "id").join(",")
    });
  };
