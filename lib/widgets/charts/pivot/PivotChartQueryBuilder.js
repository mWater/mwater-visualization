...
module.exports = PivotChartQueryBuilder = (function() {
  ...
  PivotChartQueryBuilder.prototype.createQueries = function(design, extraFilters) {
    var columnPath, columnSegment, exprCompiler, i, intersection, intersectionId, j, k, l, len, len1, len2, len3, m, queries, query, ref, ref1, relevantFilters, rowPath, rowSegment, whereClauses;
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      for (k = 0, len1 = ref1.length; k < len1; k++) {
        ...
        whereClauses = [];
        if (design.filter) {
          whereClauses.push(this.compileExpr(design.filter));
        }
