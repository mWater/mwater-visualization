    ...
    convertSpecialItemToHtml(item) {
        ...
        if (item.type === "expr") {
            ...
            if (this.summarizeExprs) {
                text = new mwater_expressions_1.ExprUtils(this.schema).summarizeExpr(exprItem.expr, this.locale);
                if (text.length > 30) {
                    text = text.substr(0, 30) + "...";
                }
                exprHtml = lodash_1.default.escape(text);
            }
            else if (lodash_1.default.has(this.exprValues, exprItem.id)) {
                // If has data
                ...
                if (value != null) {
                    ...
                    if (exprType && (0, valueFormatter_2.canFormatType)(exprType)) {
                        text = (0, valueFormatter_1.formatValue)(exprType, value, exprItem.format);
                    }
                    else {
            ...
            if (exprItem.includeLabel) {
                const label = exprItem.labelText || new mwater_expressions_1.ExprUtils(this.schema).summarizeExpr(exprItem.expr, this.locale) + ":\u00A0";
