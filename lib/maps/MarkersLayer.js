...
module.exports = MarkersLayer = (function(superClass) {
  ...
  MarkersLayer.prototype.createKMLExportJsonQL = function(design, schema, filters) {
    var axisBuilder, cluster, colorExpr, column, exprCompiler, extraFields, field, filter, geometryExpr, i, innerquery, j, k, len, len1, len2, outerquery, relevantFilters, whereClauses;
    axisBuilder = new AxisBuilder({
      schema: schema
    });
    exprCompiler = new ExprCompiler(schema);
    ...
    extraFields = ["code", "name", "desc", "type", "photos"];
    for (k = 0, len2 = extraFields.length; k < len2; k++) {
      field = extraFields[k];
      column = schema.getColumn(design.table, field);
      if (column) {
        outerquery.selects.push({
