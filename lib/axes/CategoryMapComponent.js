...
class CategoryMapComponent extends react_1.default.Component {
    constructor(props) {
        ...
        this.handleReorder = (map) => {
            ...
            return this.props.onChange(update_object_1.default(this.props.axis, { drawOrder: { $set: order } }));
        };
        this.handleColorChange = (value, color) => {
            ...
            return this.props.onChange(update_object_1.default(this.props.axis, { colorMap: { $set: colorMap } }));
        };
        this.handleExcludeChange = (value, ev) => {
            ...
            return this.props.onChange(update_object_1.default(this.props.axis, { excludedValues: { $set: excludedValues } }));
        };
        this.handleNullLabelChange = (e) => {
            const name = prompt("Enter label for none value", this.props.axis.nullLabel || "None");
            if (name) {
                return this.props.onChange(update_object_1.default(this.props.axis, { nullLabel: { $set: name } }));
            }
        };
        this.handleCategoryLabelChange = (category, e) => {
            ...
            if (name != null) {
                if (name) {
                    return this.props.onChange(immer_1.default(this.props.axis, (draft) => {
    ...
    renderLabel(category) {
        ...
        if (category.value != null) {
            return R("a", { onClick: this.handleCategoryLabelChange.bind(null, category), style: { cursor: "pointer" } }, label);
        }
        else {
            return R("a", { onClick: this.handleNullLabelChange, style: { cursor: "pointer" } }, label, R("span", { style: { fontSize: 12, marginLeft: 4 } }, "(click to change label for none value)"));
        }
    }
    renderReorderable() {
    ...
    renderToggle() {
        if (this.state.collapsed) {
            return R("div", null, R("a", { onClick: this.handleToggle }, "Show Values ", R("i", { className: "fa fa-caret-down" })));
        }
        else {
            return R("div", null, R("a", { onClick: this.handleToggle }, "Hide Values ", R("i", { className: "fa fa-caret-up" })));
        }
    }
    render() {
        if (this.state.collapsed) {
