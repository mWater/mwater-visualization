...
export default ImplicitFilterBuilder = class ImplicitFilterBuilder {
  ...
  findJoins(filterableTables) {
    ...
    for (var filterableTable of filterableTables) {
      ...
      let joins = _.filter(this.schema.getColumns(filterableTable), column => (column.type === "join") && (column.join.type === "n-1") && (column.join.toTable !== filterableTable));

      // Only keep if singular
      joins = _.flatten(_.filter(_.values(_.groupBy(joins, join => join.join.toTable)), list => list.length === 1));
      allJoins = allJoins.concat(_.map(joins, join => ({
  ...
  extendFilters(filterableTables, filters) {
    ...
    for (var join of joins) {
      const parentFilters = _.filter(filters, f => (f.table === this.schema.getColumn(join.table, join.column).join.toTable) && f.jsonql);
