...
class DatagridUtils {
        ...
        design = (0, immer_1.default)(design, (draft) => {
            for (let column of draft.columns) {
                if (column.type === "expr") {
                    ...
                    column.expr = exprCleaner.cleanExpr(column.expr ? (0, immer_1.original)(column.expr) || null : null, {
                        table,
    ...
    validateDesign(design) {
        ...
        for (const column of design.columns) {
            if (column.expr) {
                const error = new mwater_expressions_1.ExprValidator(this.schema).validateExpr(column.expr, {
                    aggrStatuses: ["individual", "literal", "aggregate"]
                });
                if (error) {
                    return error;
