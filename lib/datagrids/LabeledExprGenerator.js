...
module.exports = LabeledExprGenerator = /*#__PURE__*/function () {
      ...
      _convertColumn = function convertColumn(table, column, joins) {
        ...
        if (column.type === "id") {
          ...
          if (options.useJoinIds) {
            ...
            joinColumn = _this.schema.getColumn(column.idTable, "code");
            joinColumn = joinColumn || _this.schema.getColumn(column.idTable, "full_name");
            joinColumn = joinColumn || _this.schema.getColumn(column.idTable, "name");
            joinColumn = joinColumn || _this.schema.getColumn(column.idTable, "username");
        ...
        if (column.type === "join") {
          ...
          if ((ref = column.join.type) === "n-1" || ref === "1-1") {
            ...
            if (options.useJoinIds) {
              ...
              console.log(_this.schema.getColumns(column.join.toTable).filter(function (c) {
                return c[0] === "c";
              }));
              return _this.schema.getColumns(column.join.toTable).filter(function (c) {
                return c.id[0] === "c";
              }).map(function (c) {
                return {
                  expr: {
