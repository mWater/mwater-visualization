  ...
  FindReplaceModalComponent = class FindReplaceModalComponent extends React.Component {
    ...
    performReplace() {
      ...
      const query = {
        type: "query",
        selects: [
          { type: "select", expr: { type: "field", tableAlias: "main", column: this.props.schema.getTable(this.props.design.table).primaryKey }, alias: "id" },
          { type: "select", expr: exprCompiler.compileExpr({expr: this.state.withExpr, tableAlias: "main"}), alias: "withValue" }
        ],
      ...
      return this.props.dataSource.performQuery(query, (error, rows) => {
        ...
        return async.mapLimit(rows, 10, (row, cb) => { 
          // Abort if closed
          if (!this.state.open) {
            return; 
          }
        ...
        , (error, canEdits) => {
          ...
          return async.eachLimit(rows, 1, (row, cb) => { 
            // Abort if closed
            if (!this.state.open) {
              return; 
            }
    ...
    renderPreview() {
      ...
      design.columns = _.map(design.columns, column => {
        ...
        return _.extend({}, column, {expr, label: column.label || exprUtils.summarizeExpr(column.expr, this.props.design.locale)});
      });

      return R(AutoSizeComponent, {injectWidth: true},
        size => {
          return R(DatagridViewComponent, {
            ...
            datagridDataSource: new DirectDatagridDataSource({schema: this.props.schema, dataSource: this.props.dataSource}),
            design,
    ...
    renderContents() {
      ...
      const replaceColumns = _.filter(this.props.design.columns, column => !column.subtable && (exprUtils.getExprAggrStatus(column.expr) === "individual"));
      const replaceColumnOptions = _.map(replaceColumns, column => ({ value: column.id, label: column.label || exprUtils.summarizeExpr(column.expr, this.props.design.locale)}));
      ...
      return R('div', null,
        ...
        if (this.state.replaceColumn) {
          ...
          return R('div', {key: "with", className: "form-group"},
