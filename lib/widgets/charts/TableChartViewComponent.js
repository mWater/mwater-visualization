...
module.exports = TableChartViewComponent = (function(superClass) {
  ...
  TableChartViewComponent.prototype.componentDidUpdate = function(prevProps, prevState) {
    return this.calculateHeadersWidth();
  };

  TableChartViewComponent.prototype.componentDidMount = function() {
    return this.calculateHeadersWidth();
  };

  TableChartViewComponent.prototype.calculateHeadersWidth = function() {
    var body, bodyContainer, headers, height, tr;
    tr = $(this.refs.tableBody).find("tr").first();
    headers = $(this.refs.tableHeader).find("th");
    body = $(this.refs.tableBody);
    bodyContainer = $(this.refs.tableBodyContainer);
    tr.find("td").each((function(_this) {
      return function(i, el) {
        var cellWIdth;
        cellWIdth = $(el).width();
        headers.eq(i).width(cellWIdth);
        if (headers.eq(i).width() !== cellWIdth) {
          return _this.setColumnWidth(i, headers.eq(i).width());
        }
      };
    })(this));
    height = this.props.height * (this.props.standardWidth / this.props.width) - $(this.refs.title).outerHeight() - $(this.refs.tableHeader).outerHeight();
    return bodyContainer.height(height);
  };

  TableChartViewComponent.prototype.setColumnWidth = function(column, width) {
    var body;
    body = $(this.refs.tableBody);
    return body.find('tr').each(function(i, el) {
      return $(el).find('td').eq(column).width(width);
    });
  };
  ...
  TableChartViewComponent.prototype.render = function() {
    var containerStyle, height, style;
    style = {
