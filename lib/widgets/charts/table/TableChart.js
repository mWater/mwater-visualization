...
var AxisBuilder, Chart, ExprCompiler, ExprUtils, R, React, TableChart, TableChartViewComponent, _, injectTableAlias, isDraft, original, produce, uuid;
...
module.exports = TableChart = /*#__PURE__*/function (_Chart) {
  (0, _inherits2["default"])(TableChart, _Chart);

  var _super = _createSuper(TableChart);
  ...
  (0, _createClass2["default"])(TableChart, [{
    ...
    value: function cleanDesign(design, schema) {
      var ExprCleaner, axisBuilder, exprCleaner;
      ...
      design = produce(design, function (draft) {
        ...
        draft.version = design.version || 1; // Always have at least one column
        ...
        draft.orderings = design.orderings || []; // Clean each column

        for (columnId = j = 0, ref = draft.columns.length; 0 <= ref ? j < ref : j > ref; columnId = 0 <= ref ? ++j : --j) {
          ...
          column.textAxis = axisBuilder.cleanAxis({
            axis: column.textAxis ? original(column.textAxis) : null,
            table: design.table,
            aggrNeed: "optional"
          });
        }

        ref1 = draft.orderings; // Clean orderings

        for (k = 0, len = ref1.length; k < len; k++) {
          ordering = ref1[k];
          ordering.axis = axisBuilder.cleanAxis({
            axis: ordering.axis ? original(ordering.axis) : null,
            table: design.table,
            aggrNeed: "optional"
          });
        }
