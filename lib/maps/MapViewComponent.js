...
module.exports = MapViewComponent = (function(superClass) {
    ...
    legendItems = _.compact(_.map(this.props.design.layerViews, (function(_this) {
      return function(layerView) {
        ...
        if (layerView.visible) {
          return {
            key: layerView.id,
            legend: layer.getLegend(layerView.design, _this.props.schema)
          };
        }
      };
    })(this)));
  ...
  MapViewComponent.prototype.render = function() {
    ...
    for (index = j = 0, len = ref.length; j < len; index = ++j) {
      ...
      leafletLayer = {
        tileUrl: this.props.mapDataSource.getTileUrl(layerView.id, compiledFilters),
        utfGridUrl: this.props.mapDataSource.getUtfGridUrl(layerView.id, compiledFilters),
